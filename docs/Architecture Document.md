...
## 7.1.1 MonoBehaviour Boundary

This is a hard project rule:

- VContainer owns orchestration, input polling, scene setup, and runtime controller loops through entry points/services.
- New `MonoBehaviour` types are allowed only for Unity view bridges, serialized scene/prefab references, Unity lifecycle integration that cannot live in VContainer, and explicit debug UI/tools.
- Do not add gameplay, presenter, controller, selection, camera-input, generation, save/load, or simulation logic as `MonoBehaviour`.
- If a type does not need Unity serialization, a Unity callback, or direct view binding, it must be a plain C# service/entry point registered in VContainer.
- Scene roots are reference holders; they do not orchestrate gameplay.
...
- `GlobalSceneRoot` stores explicit scene references for renderer, debug controller, tooltip, Cinemachine orbital follow, input camera, and camera tuning
- `GlobalLifetimeScope` registers `GlobalSceneRoot` and VContainer entry points for scene initialization, camera input, and cell selection
- `GlobalMapCameraController` and `GlobalMapCellInfoController` are plain VContainer entry points/services, not `MonoBehaviour`
...
- `GlobalLifetimeScope` is missing from global scene root
- `GlobalSceneRoot` is missing config/renderer/debug/tooltip/orbital/input-camera references
...
No hardcode.
No per-bot Update.
No controller/service MonoBehaviour.
No legacy Text.
